...
# 通过将输入消息和盐组合起来进行SHAKE256哈希计算，并返回指定字节数的二进制哈希值。
...
# LatticeParameters: 处理格基参数的类。属性包括degree（多项式次数）、length（向量长度）、modulus（模数）等。方法包括初始化、相等性检查、字符串表示等。
...
class Polynomial(object):
    ...
    Attributes
    ----------
    属性包括格参数lp、常量时间标志const_time_flag、NTT表示ntt_representation等。
    ...
    Methods
    -------
    方法包括初始化、相等性检查、加法、减法、乘法、NTT变换、获取系数表示等。
        __init__(self)
            Initialize
        __eq__(self, other)
    ...
    #     """
    #     Optimized Add method for Polynomials.
    #
    #     :param other: Another Polynomial
    #     :type other: Polynomial
    ...
    #             for x, y in zip(self.ntt_representation, other.ntt_representation)
    #         ]
    #     else:
    #         result.ntt_representation = [
    #             ((x + y) % self.lp.modulus) - (
    #                 self.lp.modulus if (x + y) % self.lp.modulus > self.lp.modulus // 2 else 0)
    #             for x, y in zip(self.ntt_representation, other.ntt_representation)
    #         ]
    #     return result
    ...
    def __add__(poly1, poly2):
        """
        对两个 Polynomial 对象进行加法运算，返回一个新的 Polynomial 对象。

        :param poly1: 第一个 Polynomial 对象
        :type poly1: Polynomial
        :param poly2: 第二个 Polynomial 对象
        :type poly2: Polynomial
        :return: 加法后的新 Polynomial 对象
        :rtype: Polynomial
        """
        if poly1.lp != poly2.lp:
            raise ValueError("Lattice parameters must match for addition.")

        # 点对点加法
        return [x + y for x, y in zip(poly1.ntt_representation, poly2.ntt_representation)]
    ...
    def __sub__(poly1, poly2):
        """
        对两个 Polynomial 对象进行减法运算，返回一个新的 ntt_representation 列表。

        :param poly1: 第一个 Polynomial 对象
        :type poly1: Polynomial
        :param poly2: 第二个 Polynomial 对象
        :type poly2: Polynomial
        :return: 减法后的新的 ntt_representation 列表
        :rtype: list
        """
        if poly1.lp != poly2.lp:
            raise ValueError("Lattice parameters must match for subtraction.")
    ...
    def multiply_polynomial_by_int(poly, integer):
        ...
        result.ntt_representation = [
            cent(
                q=poly.lp.modulus,
                halfmod=poly.lp.halfmod,
                logmod=poly.lp.logmod,
                val=x * integer
            )
            for x in poly.ntt_representation
        ]
        return result
